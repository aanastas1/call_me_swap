...
- The simple algorithm uses an optimized insertion sort approach. It scans the stack from bottom to top, maintaining a sorted section at the bottom. For each element, it finds its correct position and inserts it using rotations and swaps.

- **Why O(n²)?** Each element requires a scan through the already sorted section, resulting in quadratic time complexity.

- **Best for**: Small arrays (n < 100) or arrays with low disorder (< 20%).


**2) Medium strategy (`--medium`) — target O(n√n)**

The medium algorithm uses a chunk-based approach:
- **Rank assignment**: Each element is assigned a rank based on its value
- **Chunking**: Elements are divided into chunks of size √n
- **Push phase**: Elements are pushed from `a` to `b` in chunks
- **Pull phase**: Elements are pushed back to `a` in sorted order

- **Why O(n√n)?** The chunk-based approach reduces the problem size by processing elements in groups, achieving the square root complexity.

- **Best for**: Medium-sized arrays (100-500 elements) with moderate disorder (20-50%).

**3) Complex strategy (`--complex`) — target O(n log n)**

The complex algorithm is a quicksort implementation adapted for two stacks:
- **Pivot selection**: Uses the average value as the pivot
- **Partitioning**: Elements smaller than the pivot go to `b`, larger stay in `a`
- **Recursive sorting**: Recursively sort both partitions
- **Merge**: Push elements back from `b` to `a` in sorted order

- **Why O(n log n)?** The divide-and-conquer approach splits the problem in half each time, achieving logarithmic depth.

- **Best for**: Large arrays (n > 500) or high disorder (> 50%).

**4) Complexity argument for the Adaptive strategy. How strategy selection works?**

The disorder computation is used to pick the strategy:
  - Low disorder (`disorder < 0.05`) → use `--simple` (O(n^2))
  - Medium disorder (`0.05 ≤ disorder < 0.35`) → use `--medium` (O(n√n))
...
- *aloiko - input parsing, error handling, complex algorithm, swaping functions, checker*
